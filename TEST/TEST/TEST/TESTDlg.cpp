...
void CTESTDlg::OnBnClickedOpen()
{
	...
	for (x = 0; x < height; x++) {
		for (y = 0; y < width; y++) {
			int Graylevel = (pic[y*pitch + x * 3 + 1]*0.6 + pic[y*pitch + x * 3]*0.3 + pic[y*pitch + x * 3 + 2]*0.1);
			pic[y*pitch + x * 3 + 1] = Graylevel;
			pic[y*pitch + x * 3] = Graylevel;
			pic[y*pitch + x * 3 + 2] = Graylevel;
		}
	}
	img.Draw(*pdcSrc, 0, 0);
	...
	for (i = 4; i < 255 * 3 + 1; i += 3) {
		for (j = 1; j < 255; j++) {
			pic[j*pitch + i] = (pic[(j - 1)*pitch + (i - 3)] + pic[j*pitch + (i - 3)] + pic[(j + 1)*pitch + (i - 3)] 
	...
	*/
	...
	for (i = 4; i < 255*3+1; i += 3) {
		for (j = 1; j < 255; j++) {
			pic[j*pitch + i ] = (pic[(j - 1)*pitch + (i - 3)] * (0) + pic[j*pitch + (i - 3)] * (1) + pic[(j + 1)*pitch + (i - 3)] * (0)
							+ pic[(j - 1)*pitch + i] * (1) + pic[j*pitch + i] * (-4) + pic[(j + 1)*pitch + i] * (1)
							+ pic[(j - 1)*pitch + (i + 3)] * (0) + pic[j*pitch + (i + 3)] * (1) + pic[(j + 1)*pitch + (i + 3)] * (0));

		}
	}
	img.Draw(*pdcSrc2, 0, 0);
}
